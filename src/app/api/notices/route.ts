...
import { getNotices, addNotice, updateNotice, deleteNotice, setNoticeVisible } from '@/lib/sheets'
...
export async function PATCH(req: Request) {
  ...
  if (!session?.user?.email) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  if ((session.user as any).role !== 'CO1') return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
  const { rowIndex, visible } = await req.json()
  if (!rowIndex) return NextResponse.json({ error: 'rowIndex 필수' }, { status: 400 })
  await setNoticeVisible(rowIndex, visible)
  return NextResponse.json({ success: true })
}
